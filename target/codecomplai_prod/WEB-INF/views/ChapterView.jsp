						...
						<ul class="m-0 p-0">
							<c:forEach items="${chpList}" var="chapter">
								...
								<%--
									...
									<div id="loader" class="loader" style="flex: 50%;">
									       <img class="loading-image" src="assets/images/bossloader.gif" alt="loading..">
									</div>
                                    ...
                                    <div tabindex="-1" id ="bookmarkDiv" class="col-md-12 col-12 mb-4">
                                        <h5 class="font-size-18 mb-1"><c:out value="${selectedBookTitle}"/></h5>
                                    </div>
...
function getSectionDetails(chapterId){
	 URL = "/"+$('#siteurl').val()+"/getSectionDetails?chapterId="+chapterId+"&sectionId=";
	 document.getElementById("chapterId").value=chapterId;
		$.ajax({
			...
			data: chapterId,
			success : function(response){
				console.log("sectionDetailsList...");
				console.log(response);
				if(response != null && response !=""){
					updateSectionList(chapterId,response);
					//$("#accordian").load(" #accordian");
		 			//$("#myDiv0").load(" #myDiv0");
				}
		},
		 error: function(jqXHR, textStatus, errorThrown) {
			 console.log("error in getsectiondetails ==== "+jqXHR.status + ' ' + errorThrown);
	        }
...
function updateSectionList(chapterId, sectionDetailsList) 
{
	$('#subsections').empty().append();
	for (var index in sectionDetailsList) {
      let sectionId = sectionDetailsList[index].subchapter_id;
      let sectionName = sectionDetailsList[index].subchapter_title;
      //$("#subsections").append('<li><a id='+sectionId+' onclick="showSectionDetails('+chapterId+','+sectionId+')" href="#">'+sectionName+'</a><c:choose><c:when test="${'+sectionId+' eq string2}"><div class="sidebar-submenu" style="display: block;"></div></c:when><c:otherwise><div class="sidebar-submenu" style="display: none;"></div></c:otherwise></c:choose></li>');		
  		$("#subsections").append('<li><a id='+sectionId+' onclick="showSectionDetails('+chapterId+','+sectionId+')" href="#">'+sectionName+'</a></li>');
	}
    
    //$("#subsections").show();
    $('#subsections').find('li').first().attr( 'tabIndex', -1);// make it able to focus
	$('#subsections').find('li').first().focus();//set focus on li
	$('#subsections').find('li').find('a').first().focus();// set focus to first anchor
	document.getElementById('subsections').style.display = "block";
    alert(document.getElementById('subsections').style.display);
	//$("#chapterId").focus();
	$('#sectionId').focus();
}
